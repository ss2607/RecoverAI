...
export const createItem = async (data: ItemPayload): Promise<ApiResponse<Item>> => {
  const response = await axios.post<ApiResponse<Item>>(API_URL, data);
  return response.data;
};

export const updateItem = async (id: string, data: Partial<ItemPayload>): Promise<ApiResponse<Item>> => {
  const response = await axios.put<ApiResponse<Item>>(`${API_URL}/${id}`, data);
  return response.data;
};

export const deleteItem = async (id: string): Promise<ApiResponse<Item>> => {
